...
- Small fix to add ability to show new [rush mode](https://www.counter-strike.net/newsentry/711161056325533826) maps

---

### Version 2.2.7 changelog
- Fixed debug color settings maximum value to be capped at 255
- Added error message if `dlls.mpf` is missing
- Added logging for loaded DLLs

---

### Version 2.2.6 changelog
- Added debug color preview
- Fixed crash associated with CheckForDuplicates
- Fixed mouse click coordinates not accounting for window position offset
- Fixed pixel count not resetting on non-matching pixels in accept button scan

---

### Version 2.2.5 changelog
- Fixes with workshop map icons
- Fixes with game state parsing
- Fixes with discord activity
- Compressed dll's
- Update prices according to [latest update](https://store.steampowered.com/news/app/730/view/511855214544814590)

---

### Version 2.2.4 changelog
- Fixed a race condition with portable settings
- Some DXGI edge cases fixes
- Added more debug information
- Decided to remove the mobile app entirely as it wasn't really used and mostly broken 
- Discord RPC fixes
- Some UI changes
- Map Icons fixes, using [cs2-map-icons](https://github.com/MurkyYT/cs2-map-icons)

---

### Version 2.2.3 changelog
- Added 15 seconds check in discord activity update, shouldn't spam discord now
- Made changelog look and feel better
- Some DXGI capture improvements
- Added DXGI debug info
- Update fixes

---

### Version 2.2.2 changelog
- Added translations to colors
- Made reading files in ```Steam.cs``` use ```FileShare.ReadWrite```, that way it can read the ```SteamID3``` from the log most of the times
- Added original language name alongside translated name
- Changed settings of [```Game State Integration```](https://developer.valvesoftware.com/wiki/Counter-Strike:_Global_Offensive_Game_State_Integration) in order to get information about the game faster and more reliably
- Updated third party libraries
- Fixed broken Windows 11 Rounded corners when window is maximized
- Switching to CS window only if it is not foreground already
- Saving settings on every change
- Updated [DXGICapture library](https://github.com/MurkyYT/DXGICapture) to latest version
- Fixed DXGICapture not being able to capture game window when not on primary screen
- Fixed logging crashes
- Changelog improvements 
- Latest CS2 Update broke something with GSI, fixed with current version

---

### Version 2.2.1 changelog
- Fixed logging crash
- Fixed auto buy not rebuying armor
- Auto buy optimizations
- Auto reload optimizations
- Some translations fixes and updates
- Added an indication whether the crash report was sent or not
- Log now loads even when save logs is disabled
